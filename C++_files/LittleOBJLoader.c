...
static void GenerateNormals(Mesh* mesh)
{
	...
	if (mesh->vertices && !mesh->vertexNormals)
	{
		...
		for (face = 0; face * 3 < mesh->coordCount; face++)
		{
			...
			float sqrLen0 = DotProduct(v0, v0);
			float sqrLen1 = DotProduct(v1, v1);
			float sqrLen2 = DotProduct(v2, v2);
			...
			float influence0 = DotProduct(v0, v1) / (len0 * len1);
			float influence1 = -DotProduct(v0, v2) / (len0 * len2);
			float influence2 = DotProduct(v1, v2) / (len1 * len2);
			...
			vec3 normal = CrossProduct(v0, v1);
